    ...
    for (int r = 0; r < ROWS; r++) {
        for (int c = 0; c < COLS; c++) {
            ...
            if ((r <= 3) && is_valid_cell(r, c)) {
                ...
                game_state.grid[idx] = EMPTY; // technically not necessary
            }
        }
    }
    std::copy(player_1_start, player_1_start + 10, game_state.player_1_pieces);
    std::copy(player_2_start, player_2_start + 10, game_state.player_2_pieces);
